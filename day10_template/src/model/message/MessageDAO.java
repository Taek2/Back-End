...
public class MessageDAO {
	...
	public ArrayList<MessageVO> getDBListByMember(int member){
		Connection conn = JDBC.connect();
		ArrayList<MessageVO> datas = new ArrayList();
		PreparedStatement pstmt = null;

		try {
			String sql = "select * from message where member = ? order by mnum desc"; // 최근 게시글 상단 배치
			pstmt = conn.prepareStatement(sql);
	...
	public ArrayList<MessageVO> Search(String content){
		Connection conn = JDBC.connect();
		ArrayList<MessageVO> datas = new ArrayList();
		PreparedStatement pstmt = null;

		try {
			String sql = "select * from message where writer like '"+content+"%' order by mnum desc";
			pstmt = conn.prepareStatement(sql);
			ResultSet rs = pstmt.executeQuery();
			while(rs.next()) {
				MessageVO vo = new MessageVO();
			...
			System.out.println("search()에서 출력");
			e.printStackTrace();
		}
		finally {
			JDBC.disconnect(pstmt, conn);
